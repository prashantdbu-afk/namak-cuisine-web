...
const completenessRows = foodProcessingConfig.map((record) => {
  ...
  const anchor = record.itemId
    ? `/menu#food-${record.category
        ...
        .replace(/(^-|-$)/g, "")}`
    : "—";
  return `| ${record.sourceFilename.replaceAll("|", "\\|")} | ${record.imageId} | ${record.itemId ?? "—"} | ${record.publicName} | ${record.category} | approved | ${record.status} | ${record.status === "approved" ? "yes" : "no"} | ${publicPlacement ? "yes" : "no"} | ${reason.replaceAll("|", "\\|")} | ${anchor} |`;
});
await writeFile(
  completenessPath,
